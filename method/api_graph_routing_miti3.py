...
def run_method(task, task_type, gpu_ids, model_names, hyperparameters, steers, experiment_name):
    ...
    model_path = f"logs/graph_router_{task}_{len(model_names)}.pt"
    ...
    # # Project test task embeddings if needed to match model_dim
    # if test_task_embeddings_raw.shape[1] != model_dim:
    #     if task_projection is None:
    #         task_projection = torch.nn.Linear(test_task_embeddings_raw.shape[1], model_dim).to(device)
    #     test_task_embeddings = task_projection(torch.tensor(test_task_embeddings_raw, dtype=torch.float32)).detach().cpu().numpy()
    # else:
    # test_task_embeddings = test_task_embeddings_raw
    ...
    with torch.no_grad():
        ...
        #print(edge_predict_reshaped[:3])
        scores, indices = torch.topk(edge_predict_reshaped, k=2, dim=1)
        top1_idx = indices[:, 0]
        top2_idx = indices[:, 1]
        # top1_score = scores[:, 0]
        # top2_score = scores[:, 1]
        # alpha = 0.001
        # use_top2_mask = (top1_score - top2_score) < alpha
        # final_indices = torch.where(use_top2_mask, top2_idx, top1_idx)
        selected_model_indices1 = top1_idx.cpu().numpy().tolist()
        selected_model_indices2 = top2_idx.cpu().numpy().tolist()
        # selected_model_indices = torch.argmax(edge_predict_reshaped, dim=1).cpu().numpy().tolist()
    
    assert max(selected_model_indices1) < len(model_names), "Selected model index out of range"
    assert min(selected_model_indices1) >= 0, "Selected model index out of range"
    ...
    for i in range(len(model_names)):
        ...
        for j in range(len(test_input_list)):
            if selected_model_indices1[j] == i:
                model_input_list.append(test_input_list[j])
        list_of_input_list_1.append(model_input_list)
    
    list_of_output_list_1 = distributed_generation.distributed_generation(
        model_names,
        list_of_input_list_1,
        gpu_ids,
        steers=steers,
    ...
    for i in range(len(model_names)):
        model_input_list = []
        for j in range(len(test_input_list)):
            if selected_model_indices2[j] == i:
                model_input_list.append(test_input_list[j])
        list_of_input_list_2.append(model_input_list)

    list_of_output_list_2 = distributed_generation.distributed_generation(
        model_names,
        list_of_input_list_2,
        gpu_ids,
        steers=steers,
    ...
    final_outputs_1 = []
    for j in range(len(test_input_list)):
        model_idx = selected_model_indices1[j]
        final_outputs_1.append(list_of_output_list_1[model_idx][0])
        list_of_output_list_1[model_idx].pop(0)
    
    final_outputs_2 = []
    for j in range(len(test_input_list)):
        model_idx = selected_model_indices2[j]
        final_outputs_2.append(list_of_output_list_2[model_idx][0])
        list_of_output_list_2[model_idx].pop(0)
    
    # Evaluate
    VERIFIER_PROMPT_TEMPLATE = (
    "User: ### Question: {question}\n\n"
    "### Fisrt Answer: {first_answer}\n\n"
    "### Second Answer: {second_answer}\n\n"
    "For the above question, please judge which answer is better: the first answer or wthe second answer.\n"
    "Do not solve the question by yourself; just judge which answer is better to route.\n"
    "If the first answer is better, output \"Final Decision: 1\". If the second answer is better, output \"Final Decision: 2\". Assistant:"
    )
    ...
    judge_input = []
    for j in range(len(test_input_list)):
        current_prompt = VERIFIER_PROMPT_TEMPLATE.format(
            question=test_input_list[j],
            first_answer=final_outputs_1[j],
            second_answer=final_outputs_2[j]
        )
    ...
    for output in judge_output:
        if VERIFIER_1_TAG in output:
            select_1_or_2.append(1)
        else:
            select_1_or_2.append(2)
    ...
    selected_model_indices = []
    for j in range(len(test_input_list)):
        if select_1_or_2[j] == 1:
            final_outputs.append(final_outputs_1[j])
            selected_model_indices.append(selected_model_indices1[j])
        else:
            final_outputs.append(final_outputs_2[j])
            selected_model_indices.append(selected_model_indices2[j])
    # print(final_outputs)
