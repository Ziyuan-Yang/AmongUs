...
#def run_method(task, task_type, gpu_ids, model_names, hyperparameters, prompts, steers, experiment_name):
    ...
    router_model_name = "logs/router_sft_initial6"

    # using the router for the test set
    test_input_list = eval.prepare_inputs(task, task_type, "test")
    # Top-1
    selected_model_indices1 = []
    ...
    for output in list_of_output_list:
        ...
        selected_model_indices1.append(model_index)
    ...
    # Top-2
    selected_model_indices2 = []
    router_prompts = []
    for i in range(len(test_input_list)):
        mask_idx = selected_model_indices1[i]
        if model_descriptions is not None:
            prompt = "You are an AI assistant tasked with choosing the best model to answer the user's question based on the model descriptions.\n\n"
            prompt += "Model Descriptions:\n"
            for m in range(len(model_names)):
    ...
    for output in list_of_output_list:
        ...
        if not found:
            model_index = random.randint(0, len(model_names) - 1)
        selected_model_indices2.append(model_index)
    assert max(selected_model_indices2) < len(model_names), "Selected model index out of range"
    assert min(selected_model_indices2) >= 0, "Selected model index out of range"
    ...
    for i in range(len(model_names)):
        ...
        list_of_input_list1.append(model_input_list)

    list_of_output_list1 = distributed_generation.distributed_generation(
        model_names,
        list_of_input_list1,
        gpu_ids,
        steers=steers,
    ...
    for i in range(len(model_names)):
        ...
        for j in range(len(test_input_list)):
            if selected_model_indices2[j] == i:
                model_input_list.append(test_input_list[j])
        list_of_input_list2.append(model_input_list)

    list_of_output_list2 = distributed_generation.distributed_generation(
        model_names,
        list_of_input_list2,
        gpu_ids,
        steers=steers,
    ...
    final_outputs1 = []
    for j in range(len(test_input_list)):
        final_outputs1.append(list_of_output_list1[selected_model_indices1[j]][0])
        # pop the used output
        list_of_output_list1[selected_model_indices1[j]].pop(0)
    
    final_outputs2 = []
    for j in range(len(test_input_list)):
        final_outputs2.append(list_of_output_list2[selected_model_indices2[j]][0])
        # pop the used output
        list_of_output_list2[selected_model_indices2[j]].pop(0)
    ...
    for j in range(len(test_input_list)):
        current_prompt = VERIFIER_PROMPT_TEMPLATE.format(
            ...
            first_answer=final_outputs1[j],
            second_answer=final_outputs2[j]
        )
    ...
    for j in range(len(test_input_list)):
        if select_1_or_2[j] == 1:
            final_outputs.append(final_outputs1[j])
            selected_model_indices.append(selected_model_indices1[j])
        else:
            final_outputs.append(final_outputs2[j])
